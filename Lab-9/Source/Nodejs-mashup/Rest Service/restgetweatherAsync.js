/**
 * Created by Harmeet on 22/03/2017. MashUp of REST APIs
 */
...
app.get('/getEventsWithWeather/:location', function (req, res) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            request('http://api.eventful.com/json/events/search?app_key=FpC7XWgJX2xPLHb5C&location='+req.params.location+'&date=Today', function (error, response, body)
            {
                //Check for error
                if(error){
        ...
        function(callback) {
            ...
            for(var i=0;i<result.eventsList.length;i++)
            {
                ...
                currentEvent=event;
                request('http://api.wunderground.com/api/00026fa5df7a27f0/conditions/q/' + event.latitude + ',' + event.longitude + '.json', function (error, response, body)
                {
                    if (error) {
